...
printf 'Publish ast at %s to %s? [y/N] ' "$(node -p "require('./packages/tool-ast/package.json').version")" "$registry"
read -r answer
...
(cd packages/tool-ast && pnpm publish --access public --registry "$registry" --no-git-checks)
